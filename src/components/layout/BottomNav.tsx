...
export function BottomNav() {
  ...
  const chefLinks = [
    { to: "/chef", label: t("nav.roleChef"), icon: ChefHat, end: true },
    { to: "/home", label: t("nav.feed"), icon: Home },
    { to: "/add-recipe", label: t("nav.create"), icon: PlusCircle },
    { to: "/meal-planner", label: t("nav.plans"), icon: CalendarDays },
        ...
        {chefLinks.map(({ to, label, icon, end }) => (
          <li key={to}>
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  "relative flex min-h-14 flex-col items-center justify-center gap-0.5 px-0.5 text-center text-[11px] font-medium leading-tight transition-colors duration-150",
