...
import { isRecipePublicId, type RecipeResponse } from "../../types/recipe.types"
...
export default function RecipeEditorPage() {
  ...
  usePageMeta({
    ...
    path: isEdit && publicId ? `/edit-recipe/${publicId}` : "/add-recipe",
  })
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true)
      if (!isRecipePublicId(publicId)) {
        setLoadError("Identifiant de recette invalide.")
        setLoading(false)
        return
      }
      try {
        const data = await recipeService.getRecipeByPublicId(publicId)
        if (cancelled) return
  ...
  const uploadImageIfNeeded = async (id: string, image?: File | null) => {
    if (!image) return
    const formData = new FormData()
    formData.append("imageUrl", image)
    await recipeService.addImageToRecipe(id, formData)
  }

  const handleSubmit = async (recipeData: RecipeFormData, image?: File | null) => {
    if (isEdit && publicId) {
      await recipeService.updateRecipe(publicId, recipeData)
      await uploadImageIfNeeded(publicId, image)
    ...
    if (created.publicId) {
      await uploadImageIfNeeded(created.publicId, image)
    }
