...
export default function CommunityPage() {
  ...
  const [reportTarget, setReportTarget] = useState<
    null | { kind: "recipe"; publicId: string } | { kind: "comment"; publicId: string; commentId: number }
  >(null)
  ...
  const [recipePublicId, setRecipePublicId] = useState<string | null>(null)
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      await addComment(activeRecipe.publicId ?? "", commentText)
      setCommentText("")
  ...
  const openCommentDialog = (recipe: RecipeResponse) => {
    ...
    fetchComments(recipe.publicId ?? "")
    setCommentDialogOpen(true)
  }

  const handleAddImageClick = (publicId: string) => {
    setRecipePublicId(publicId)
    setIsPopupOpen(true)
  }
    ...
    try {
      await runOptimisticMutation({
        applyOptimistic: () => {
          ...
          setFeedRecipes((prev) => prev.filter((recipe) => recipe.id !== target.id))
          return snapshot
        },
        mutation: () => recipeService.deleteRecipe(target.publicId as string),
  ...
  const handleOptimisticLike = async (recipeId: number) => {
    ...
    try {
      await runOptimisticMutation({
        ...
        mutation: () => recipeService.toggleLike(currentRecipe.publicId as string).then(() => undefined),
        rollback: (snapshot) => setFeedRecipes(snapshot),
        onError: (_error, retry) => {
  ...
  const handleOptimisticBookmark = async (recipeId: number) => {
    ...
    try {
      await runOptimisticMutation({
        ...
        mutation: () =>
          nextIsSaved
            ? recipeService.saveRecipe(currentRecipe.publicId as string)
            : recipeService.unsaveRecipe(currentRecipe.publicId as string),
        rollback: (snapshot) => setFeedRecipes(snapshot),
        onSuccess: () => {
          notifySuccess(
  ...
  const handleReportRecipe = (publicId: string) => {
    setReportTarget({ kind: "recipe", publicId })
  }

  const handleReportComment = (publicId: string, commentId: number) => {
    setReportTarget({ kind: "comment", publicId, commentId })
  }
  ...
  const submitReport = async (reason: string) => {
    ...
    try {
      if (reportTarget.kind === "recipe") {
        const response = await recipeService.reportRecipe(reportTarget.publicId, reason.trim())
    ...
    <AppShell>
                        ...
                        >
                          <div className={cn("relative", index === 0 ? "h-64 md:h-80" : "h-48")}>
                            {recipe.imageUrl ? (
                              ...
                              <div
                                onClick={() => recipe.publicId && handleAddImageClick(recipe.publicId)}
                            ...
                            <div className="absolute top-2 right-2 p-2 flex space-x-1">
                              ...
                              <button
                                ...
                                onClick={(event) => {
                                  event.preventDefault()
                                  event.stopPropagation()
                                  if (recipe.publicId) handleReportRecipe(recipe.publicId)
                                }}
          ...
          <div className="max-h-[60vh] overflow-y-auto py-4">
            {commentsLoading ? (
              ...
              <div className="text-center py-8">
                ...
                <Button variant="outline" size="sm" onClick={() => activeRecipe?.publicId && fetchComments(activeRecipe.publicId)}>
                  Réessayer
                </Button>
              </div>
            ) : comments.length === 0 ? (
                        ...
                        <div className="flex items-center text-muted-foreground">
                          ...
                          {activeRecipe && (
                            <button
                              ...
                              onClick={() => activeRecipe.publicId && handleReportComment(activeRecipe.publicId, comment.id)}
      ...
      <AddRecipeDialog
        ...
        onSubmit={async (recipeData, image) => {
          ...
          if (image && recipe?.publicId) {
            const formData = new FormData()
            formData.append("imageUrl", image)
            await recipeService.addImageToRecipe(recipe.publicId as string, formData)
          }
          return recipe?.id ?? 0
        }}
      />
      ...
      {recipePublicId && isPopupOpen && (
        <ImageUploadDialog open={isPopupOpen} onOpenChange={setIsPopupOpen} recipePublicId={recipePublicId} />
      )}
